...
}, [lastExpense]); // NOTE: keep as is? (your original had [lastExpense], but we won't touch logic)
          ...
          <div className="max-w-5xl mx-auto">
            <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
              ...
              <div className="text-sm text-slate-600 mt-2">
                Нямаш достъп до финансовите справки. Тази секция е достъпна само за домоуправител.
              </div>
            </div>
          </div>
        </div>
    ...
    <div className="min-h-screen flex flex-col bg-slate-50">
      <div className="flex-1 p-6">
        <div className="max-w-5xl mx-auto space-y-6">
          ...
          <div className="rounded-3xl bg-white p-6 shadow-sm border border-slate-200">
            ...
            <div className="mt-4 rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-700">
              ...
              <ul className="mt-2 list-disc pl-5 space-y-1">
                ...
                <li>
                  След заключване, IBAN и получателят <b>не могат да се променят</b> от домоуправителя (само Админ може да
            ...
            <div className="mt-5 rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-700">
              ...
              <div className="mt-2 flex flex-wrap gap-2">
                <a href="#summary" className="rounded-full border border-slate-300 px-3 py-1 hover:bg-slate-100 transition">
                  Обобщение
                </a>
                <a href="#how" className="rounded-full border border-slate-300 px-3 py-1 hover:bg-slate-100 transition">
                  Как работи
                </a>
                <a href="#payout" className="rounded-full border border-slate-300 px-3 py-1 hover:bg-slate-100 transition">
                  Данни за получаване
                </a>
                <a href="#balance" className="rounded-full border border-slate-300 px-3 py-1 hover:bg-slate-100 transition">
                  Баланс
                </a>
                <a href="#expenses" className="rounded-full border border-slate-300 px-3 py-1 hover:bg-slate-100 transition">
                  Разходи
                </a>
                <a href="#policy" className="rounded-full border border-slate-300 px-3 py-1 hover:bg-slate-100 transition">
                  Политика и отчетност
                </a>
                <a href="#faq" className="rounded-full border border-slate-300 px-3 py-1 hover:bg-slate-100 transition">
                  Въпроси
                </a>
              </div>
            </div>
          ...
          {/* Summary */}
          <div id="summary" className="rounded-3xl bg-white p-6 shadow-sm border border-slate-200">
            ...
            <div className="mt-4 grid grid-cols-1 md:grid-cols-5 gap-3 text-sm">
              ...
              <div className="rounded-2xl border border-slate-200 p-4">
                ...
                <div className="mt-1 text-lg font-bold text-slate-900">{locked ? "Заключено" : "Не е заключено"}</div>
                <div className="text-xs text-slate-500 mt-1">
                  {locked ? "Stripe плащанията увеличават баланса при успешно преминало плащане." : "За автоматично отчитане, първо заключи данните."}
                </div>
              </div>
            </div>
          ...
          {/* How it works */}
          <div id="how" className="rounded-3xl bg-white p-6 shadow-sm border border-slate-200">
          ...
          {/* Payout details */}
          <div id="payout" className="rounded-3xl bg-white p-6 shadow-sm border border-slate-200">
            <div className="flex items-start justify-between gap-4">
              ...
              {locked ? (
                <span className="shrink-0 rounded-full bg-emerald-50 border border-emerald-200 px-3 py-1 text-xs font-semibold text-emerald-900">
                  Заключено
                </span>
              ) : (
                <span className="shrink-0 rounded-full bg-amber-50 border border-amber-200 px-3 py-1 text-xs font-semibold text-amber-900">
                  Не е заключено
                </span>
              )}
            </div>
            ...
            <div className="mt-4 grid grid-cols-1 sm:grid-cols-3 gap-3">
              <div className="space-y-1">
                ...
                <input
                  ...
                  className="w-full border border-slate-200 rounded-2xl px-4 py-3 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-slate-300 disabled:bg-slate-100"
                />
                <div className="text-xs text-slate-500">Пример: “Сдружение на собствениците – Вход А”</div>
              </div>
              ...
              <div className="space-y-1">
                ...
                <input
                  ...
                  className="w-full border border-slate-200 rounded-2xl px-4 py-3 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-slate-300 disabled:bg-slate-100"
                />
                <div className="text-xs text-slate-500">Ако започвате от 0, въведете 0 и заключете.</div>
              </div>
            </div>
            ...
            {!locked && (
              <div className="mt-4 flex flex-col sm:flex-row gap-3 sm:items-center sm:justify-between">
                ...
                <button
                  ...
                  className="rounded-2xl px-5 py-3 bg-slate-900 text-white font-semibold text-sm hover:bg-slate-800 transition shadow-sm"
                >
                  Запази и заключи
                </button>
              </div>
          ...
          {/* Balance */}
          {locked && (
            <div id="balance" className="rounded-3xl bg-white p-6 shadow-sm border border-slate-200">
              <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                ...
                <div className="text-3xl font-black text-slate-900">{Number(balance || 0).toFixed(2)} €</div>
              </div>

              <div className="mt-4 grid grid-cols-1 md:grid-cols-3 gap-3 text-sm">
                <div className="rounded-2xl border border-slate-200 p-4">
                  <div className="text-xs font-semibold text-slate-600">Общо разходи</div>
                  <div className="mt-1 text-lg font-bold text-slate-900">{Number(expensesTotal || 0).toFixed(2)} €</div>
                </div>
                <div className="rounded-2xl border border-slate-200 p-4">
                  <div className="text-xs font-semibold text-slate-600">Брой разходи</div>
                  <div className="mt-1 text-lg font-bold text-slate-900">{Array.isArray(expenses) ? expenses.length : 0}</div>
                </div>
                <div className="rounded-2xl border border-slate-200 p-4">
                  <div className="text-xs font-semibold text-slate-600">Статус</div>
                  <div className="mt-1 text-lg font-bold text-slate-900">{locked ? "Заключено" : "Не е заключено"}</div>
          ...
          {/* Expenses */}
          {locked && (
            <div id="expenses" className="rounded-3xl bg-white p-6 shadow-sm border border-slate-200">
              ...
              <div className="mt-4 grid grid-cols-1 sm:grid-cols-3 gap-3">
                ...
                <div className="space-y-1">
                  ...
                  <input
                    ...
                    className="w-full border border-slate-200 rounded-2xl px-4 py-3 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-slate-300"
                  />
                  <div className="text-xs text-slate-500">Пример: “Смяна на осветление на етажа”</div>
                </div>

                <div className="flex items-end">
                  <button
                    onClick={addExpense}
                    className="w-full rounded-2xl px-4 py-3.5 bg-slate-900 text-white font-semibold text-sm hover:bg-slate-800 transition shadow-sm"
                  >
                    Добави разход
                  </button>
                </div>
                  ...
                  <>
                    <div className="mt-3 rounded-2xl border border-slate-200 overflow-hidden">
                      ...
                      {expensesSorted.map((e, i) => (
                        <div key={i} className="grid grid-cols-12 gap-2 border-t px-4 py-3 text-sm items-center">
                          ...
                          <div className="col-span-6 min-w-0">
                            <div className="font-semibold text-slate-900 truncate">{e.description || "Разход"}</div>
                          </div>
          ...
          {/* Policy */}
          <div id="policy" className="rounded-3xl bg-white p-6 shadow-sm border border-slate-200">
          ...
          {/* FAQ */}
          <div id="faq" className="rounded-3xl bg-white p-6 shadow-sm border border-slate-200">
            ...
            <div className="mt-3 space-y-3 text-sm text-slate-700">
              <div className="rounded-2xl border border-slate-200 p-4">
                ...
                <div className="mt-1">Не. Това е вътрешен счетоводен баланс за нуждите на отчетността в системата.</div>
              </div>
              ...
              <div className="rounded-2xl border border-slate-200 p-4">
                ...
                <div className="mt-1">При успешно плащане през Stripe (checkout.session.completed webhook) и когато финансите са заключени.</div>
              </div>
